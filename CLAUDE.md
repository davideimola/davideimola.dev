...
- **Column split:** the left rail is only the history (Experience, Freelance work, Community & projects). Selected talks and Events organised live in the **right** column under the aside card, on screen as well as on paper, which is what stops the space under the card printing blank. One structure for both media on purpose: moving them for print only would mean coordinating grid rows by hand, and that breaks as soon as an engagement type empties out and its section renders nothing.
