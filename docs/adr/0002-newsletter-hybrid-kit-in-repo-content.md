...
- **Two pre-build verifications** flagged by the research were run and both **confirmed** by the [Kit free-tier spike](../research/kit-api-verification-spike.md) (issue #71): (a) broadcast **sending** via API works on the free Newsletter plan (`send_at` on `POST /v4/broadcasts` → `scheduled`, no plan block); (b) double opt-in **is** achievable on v4 — not via a bare subscriber-create, but through a **two-call flow** (`POST /v4/subscribers` with `state: "inactive"`, then `POST /v4/forms/{id}/subscribers` against a double-opt-in-enabled form, which sends Kit's confirmation email). No v3 API or embedded Kit form needed; this is the flow the subscribe capability (#74) builds.
- **Deliverability prerequisite (before issue #1 ships):** the spike's test send delivered but landed in spam, and `GET /v4/account` reported the sending address as `is_dmarc_configured: false`. **Kit domain authentication for `davideimola.dev` (DKIM CNAMEs from Kit, SPF include, DMARC alignment) must be completed and placement re-tested before the first real issue** — separate from the (proven) ability to send via the API.
